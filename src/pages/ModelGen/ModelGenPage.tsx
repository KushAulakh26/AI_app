...
import type { useModelGen } from './useModelGen'
import { MgTopBar } from '@/components/model-gen/MgTopBar'
import { MgStepBar } from '@/components/model-gen/MgStepBar'
import { MgUploadPanel } from '@/components/model-gen/MgUploadPanel'
import { MgModePanel } from '@/components/model-gen/MgModePanel'
import { MgModelPicker } from '@/components/model-gen/MgModelPicker'
import { MgResultPanel } from '@/components/model-gen/MgResultPanel'
import { MgHistorySidebar } from '@/components/model-gen/MgHistorySidebar'
import { CostConfirmDialog } from '@/components/billing/CostConfirmDialog'

export function ModelGenPage(p: ReturnType<typeof useModelGen>) {
  return (
    <div className="relative min-h-screen overflow-x-clip bg-background text-foreground">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pt-10 lg:px-8">
        <p className="font-mono text-xs uppercase tracking-widest text-primary">Model Studio</p>
        <h1 className="mt-2 text-4xl font-bold leading-tight lg:text-5xl">
          AI 模特圖<span className="text-primary">生成</span>
        </h1>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground lg:text-base">
          上傳商品圖自動摳出服裝，四種玩法任選：換模特、指定模特、自動出 4 張、整套穿搭。
          生圖模型隨時切換，每次生成都會留在右側歷史裏，方便翻看對比。
        </p>
        <div className="mt-6">
          <MgStepBar stepIndex={p.stepIndex} />
        </div>
      </div>
