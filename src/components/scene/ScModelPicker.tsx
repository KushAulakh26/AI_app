...
import { modelMetaOf } from '@/pages/Scene/useScene'
...
export function ScModelPicker(p: ReturnType<typeof useScene>) {
  return (
      ...
      <div className="mt-5 flex flex-col gap-3 rounded-xl bg-secondary/60 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-medium">
            場景合成 · 本次出 <span className="font-bold text-primary">{p.taskCount}</span> 張
          </p>
          ...
          {!p.cutoutUrl && p.cutoutJobStatus !== 'cutting' && (
            <p className="mt-1 text-xs text-muted-foreground">先完成上傳與摳圖，並至少選擇一個場景</p>
          )}
        </div>
        <button
          type="button"
