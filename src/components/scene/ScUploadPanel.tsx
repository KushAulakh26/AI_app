...
export function ScUploadPanel(p: ReturnType<typeof useScene>) {
    ...
    <section className="rounded-2xl border border-border bg-card p-5 shadow-md">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold">上傳商品圖，自動摳出商品</h2>
        ...
        >
          {p.imageUrlsPreview ? (
            ...
            <div className="flex flex-col items-center gap-3 text-muted-foreground">
              ...
              <p className="text-xs">背景乾淨的平鋪或實拍商品圖，效果更佳</p>
            </div>
          )}
        ...
        <div className="flex min-h-56 flex-col rounded-xl border border-border bg-muted/60 p-4">
          ...
          {p.cutoutJobStatus === 'cutting' && (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 text-muted-foreground">
              ...
              <p className="text-sm">正在摳出商品，約需 30–90 秒…</p>
            </div>
          )}
          ...
          {p.cutoutJobStatus === 'idle' && (
            <div className="flex flex-1 items-center justify-center text-center text-sm text-muted-foreground">
              {p.imageUrlsStatus === 'done' ? '準備開始摳圖…' : '上傳商品圖後，這裏會展示摳好的透明底商品'}
            </div>
          )}
        </div>
      </div>
